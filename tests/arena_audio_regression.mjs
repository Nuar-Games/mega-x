...
for (const name of ['card-selected','card-draw','effect-enter-field','card-enter-vs','card-attacking','card-destroyed','card-goes-to-zon-x','prompt-needed','player-win']) must(assets.includes(`/audio/replacements/${name}.opus`), `replacement audio missing: ${name}`)
...
console.log('PASS arena audio: dedicated VS-entry SFX, Effect-only entry SFX, no FIGHT or Arena-entry announcer path')
