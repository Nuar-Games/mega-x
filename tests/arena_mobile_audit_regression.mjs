...
must(audio.includes(".motion-card-fx.draw')) kind = 'draw'"), 'draw event SFX is not wired')
must(audio.includes(".motion-card-fx.enter_vs, .motion-card-fx.support')) kind = 'enter'"), 'card-entry event SFX is not wired')
must(audio.includes(".motion-card-fx.destroy')) kind = 'destroy'"), 'destroy event SFX is not wired')
must(audio.includes(".combat-screen-fx.stage-impact')) kind = 'attack'"), 'attack event SFX is not wired')
must(audio.includes('this.playSfx(kind)'), 'arena event SFX dispatcher missing')
