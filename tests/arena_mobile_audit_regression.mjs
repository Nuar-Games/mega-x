...
must(fragment.includes("activeOnlineMatch ? actionTimerIndex === localViewer : game.effectTurn === bottomPlayer"),'authoritative online Effect prompt gate missing')
must(fragment.includes("activeOnlineMatch ? actionTimerIndex === localViewer : game.attackTurn === bottomPlayer"),'authoritative online Attack prompt gate missing')
...
console.log('PASS Arena MX3 final lock: authoritative action-timer prompts, supplied VS art/flare, compact prompt, contained opponent hand, dedicated VS-entry SFX, one-shot result music, no FIGHT audio')
