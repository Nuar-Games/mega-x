...
must(fragment.includes("activeOnlineMatch ? game.effectTurn === localViewer : game.effectTurn === bottomPlayer"),'authoritative online Effect prompt gate missing')
must(fragment.includes("activeOnlineMatch ? game.attackTurn === localViewer : game.attackTurn === bottomPlayer"),'authoritative online Attack prompt gate missing')
...
console.log('PASS Arena MX3 final lock: authoritative game-turn prompts, supplied VS art/flare, compact prompt, contained opponent hand, dedicated VS-entry SFX, one-shot result music, no FIGHT audio')
