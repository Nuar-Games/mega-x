...
must(fragment.includes("<span>{bottomPlayer === 0 ? 'ANDA' : 'LAWAN'}</span><strong>{playerDisplayName(0)}</strong>"), 'left fighter ownership must follow local player identity')
must(fragment.includes("<span>{bottomPlayer === 1 ? 'ANDA' : 'LAWAN'}</span><strong>{playerDisplayName(1)}</strong>"), 'right fighter ownership must follow local player identity')
must(fragment.includes("<span>{bottomPlayer === 0 ? 'VS ANDA' : 'VS LAWAN'}</span>"), 'left VS ownership label must be player-relative')
must(fragment.includes("<span>{bottomPlayer === 1 ? 'VS ANDA' : 'VS LAWAN'}</span>"), 'right VS ownership label must be player-relative')
