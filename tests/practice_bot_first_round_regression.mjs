import { startPracticeMatch, submitPracticeAction, PRACTICE_BOT_ID } from '../src/practice-match.ts'

const humanId = 'practice-bot-first-test'
let match = null
let cardId = null
for (let attempt = 0; attempt < 80; attempt += 1) {
  ...
  if (candidate.state.firstPlayer === PRACTICE_BOT_ID && nonChoiceCardId !== undefined) {
    match = candidate
...
if (!match || cardId === null) throw new Error('could not produce a bot-first practice match with a non-SPUDUR opening VS')
...
if (result.phase !== 'EFFECT') {
  ...
  throw new Error(`practice bot-first dead turn reproduced: expected EFFECT after both VS were set, got ${result.phase}; state=${JSON.stringify(diag)}`)
}
if (result.state.effectTurn !== PRACTICE_BOT_ID) {
  throw new Error(`expected Beginner Bot Effect turn after automatic round start, got ${result.state.effectTurn}`)
}

console.log('PASS Practice automatically begins the round when the bot is first player')
