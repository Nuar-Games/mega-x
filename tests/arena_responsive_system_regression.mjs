...
must(!auditPatch.includes('transform:scale(.72)'), 'arena audit patch still contains phone-only HUD scaling')
must(!auditPatch.includes('.duel-shell .battlefield{top:72px'), 'arena audit patch still owns battlefield geometry')
...
must(css.includes('grid-template-rows:var(--mx-opponent-hand-h) minmax(0,1fr) var(--mx-local-hand-h)'), 'arena shell does not reserve both hands and battlefield')
must(css.includes('@container (max-aspect-ratio: 3/4)'), 'portrait container contract missing')
must(css.includes('@container (min-aspect-ratio: 4/3)'), 'wide container contract missing')
...
must(css.includes('.duel-shell .mx-responsive-vs-title,.duel-shell .mx-responsive-discard-title{position:relative!important'), 'selection titles are not isolated from board geometry')
