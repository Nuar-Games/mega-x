...
must(bridge.includes('tagDiscardSelection(panel, title)'), 'Android discard flow does not receive premium tray behavior')
must(bridge.includes('mx3-discard-selection-grid') && bridge.includes('mx3-discard-choice-item'), 'Android discard card-grid tagging missing')
must(bridge.includes('mx3DiscardSummary') && bridge.includes('dipilih'), 'Android discard selected/required counter missing')
...
must(css.includes('Android-only premium discard selection tray'), 'premium Android discard tray styling missing')
must(css.includes('grid-template-columns:repeat(2,minmax(0,1fr))'), 'Android discard cards are not arranged in a compact two-column grid')
must(css.includes("content:'✓ DIPILIH'"), 'selected discard card premium badge missing')
must(css.includes('.mx3-discard-selection-footer') && css.includes('position:sticky!important'), 'discard confirmation footer is not kept visible')

console.log('PASS Arena usability: functional AUDIO panel, lowered hand, and premium Android-only discard selection tray are locked')
