...
function findBalancedJsxExpression(source, start) {
  ...
  for (let i = start; i < source.length; i += 1) {
    ...
    if (mode === 'template') {
      ...
      if (ch === '$' && next === '{') { depth += 1; templateReturnDepth.push(depth - 1); mode = 'code'; i += 1 }
      continue
    }
    ...
    if (ch === '}') {
      depth -= 1
      if (templateReturnDepth.length && depth === templateReturnDepth[templateReturnDepth.length - 1]) { templateReturnDepth.pop(); mode = 'template' }
...
for (const required of ['fighter-hud','PlayerHand','arena-wrap','action-bar']) if (!removedRegion.includes(required)) throw new Error(`Arena 2 rebuild: expected legacy region token missing: ${required}`)
app = app.slice(0, headerStart) + replacement + app.slice(actionEnd)

// Playing from hand happens through the selected-card overlay, not five rows of tiny inline buttons.
...
while (true) {
  ...
  if (end < 0) throw new Error('Arena 2 rebuild: legacy focused-card inspector closing brace missing')
...
app = app.replace(oldIntro, `{arenaIntro && <div className="mx2-entry-flash" aria-hidden="true"><i></i><i></i></div>}`)
