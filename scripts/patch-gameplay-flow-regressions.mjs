...
const replacements=[
  ...
  ["(game.phase === 'EFFECT' && game.effectTurn === bottomPlayer)","(game.phase === 'EFFECT' && game.effectTurn !== null && (activeOnlineMatch ? game.effectTurn === onlineSession?.userId : game.effectTurn === bottomPlayer))"],
  ["(game.phase === 'ATTACK' && game.attackTurn === bottomPlayer)","(game.phase === 'ATTACK' && game.attackTurn !== null && (activeOnlineMatch ? game.attackTurn === onlineSession?.userId : game.attackTurn === bottomPlayer))"],
  ["{game.phase === 'EFFECT' && game.effectTurn === bottomPlayer && <button onClick={requestEndEffectTurn}","{game.phase === 'EFFECT' && game.effectTurn !== null && (activeOnlineMatch ? game.effectTurn === onlineSession?.userId : game.effectTurn === bottomPlayer) && <button onClick={requestEndEffectTurn}"],
  ["{game.phase === 'ATTACK' && game.attackTurn === bottomPlayer && <>","{game.phase === 'ATTACK' && game.attackTurn !== null && (activeOnlineMatch ? game.attackTurn === onlineSession?.userId : game.attackTurn === bottomPlayer) && <>"],
]
...
arena=arena.replace(/canBegin && passToPlayer === null && \(!activeOnlineMatch \|\| [^\n]+?\)/g,()=>{
  beginRoundRepairs+=1
  return 'canBegin && passToPlayer === null && (!activeOnlineMatch || game.firstPlayer === onlineSession?.userId)'
})
...
console.log('Repaired Kapores persistence, zero-target choice deadlocks, and UUID-correct Arena SET_VS/Effect/Attack/begin-round prompts')
