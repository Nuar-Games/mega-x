...
export class ArenaPrototypeScene extends Phaser.Scene {
  ...
  consumeEvent(envelope:ArenaEventEnvelope,nextState:ArenaState):Promise<void>{
    ...
    const hand=current?.players[event.player].hand??[]
    const cardIndex=Math.max(0,hand.findIndex(card=>card.id===event.cardId))
    const start=this.handPoint(cardIndex,Math.max(1,hand.length),layout)
    const destination=layout.vs[event.player]
    const moving=this.add.container(start.x,start.y)
    const body=this.add.rectangle(0,0,destination.width*0.62,destination.height*0.62,PANEL,1).setStrokeStyle(2,LINE)
    const label=this.add.text(0,0,hand[cardIndex]?.name??`CARD ${event.cardId}`,{fontFamily:'Arial, sans-serif',fontSize:'14px',color:TEXT,align:'center',wordWrap:{width:destination.width*0.52}}).setOrigin(0.5)
    moving.add([body,label])
    moving.setDepth(100)

    return new Promise(resolve=>{
      this.tweens.add({
        targets:moving,x:destination.x,y:destination.y,duration:480,ease:'Cubic.easeInOut',
        onComplete:()=>{
          moving.destroy(true)
          this.rebuildFromState(nextState)
          resolve()
