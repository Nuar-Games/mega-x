...
export class ArenaHud{
  ...
  private combatStats(x:number,y:number,stats:ArenaStats,position:string,align:'left'|'right',accent:number,base:number){
    ...
    g.lineStyle(Math.max(1,base*0.07),accent,0.66).lineBetween(x,y,x+dir*span,y)
  ...
  render(state:ArenaRenderState,layout:ArenaLayoutSnapshot){
    ...
    const plateW=compact?layout.hud.width*0.49:Math.min(layout.hud.width*0.43,650)
    const plateH=compact?layout.hud.height*0.94:Math.min(layout.hud.height*1.22,145)
