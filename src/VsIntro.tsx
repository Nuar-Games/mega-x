...
export function VsIntroScreen({session,match,onComplete,onError}:Props){
  const [stage,setStage]=useState<VsIntroStage>('ENTRY'),timersRef=useRef<number[]>([]),audioRef=useRef<HTMLAudioElement|null>(null),startedRef=useRef(false),completedRef=useRef(false)
  useEffect(()=>{let disposed=false,startedMatch:ActiveOnlineMatch|null=null;const introStartedAt=performance.now();const finish=()=>{if(disposed||completedRef.current||startedMatch===null)return;completedRef.current=true;onComplete(startedMatch)};const schedule=(delay:number,action:()=>void)=>{const timer=window.setTimeout(()=>{if(!disposed)action()},delay);timersRef.current.push(timer)};const audio=new Audio(VS_INTRO_AUDIO_SRC);audio.preload='auto';audio.volume=.28;audioRef.current=audio;const playIntroSegment=()=>{if(disposed)return;audio.currentTime=VS_INTRO_AUDIO_START;void audio.play().catch(()=>undefined)};if(audio.readyState>=1)playIntroSegment();else audio.addEventListener('loadedmetadata',playIntroSegment,{once:true});audio.load();setStage('ENTRY');schedule(2500,()=>setStage('VERSUS'));schedule(7000,()=>setStage('FIGHT'));schedule(8400,()=>{setStage('REVEAL');if(startedRef.current)return;startedRef.current=true;void startVsIntroMatch(session,match.id).then(()=>getMyActiveMatchVsIntro(session)).then(activeMatch=>{if(disposed)return;if(!activeMatch||activeMatch.status!=='ACTIVE')throw new Error('MATCH_START_NOT_ACTIVE');startedMatch=activeMatch;if(performance.now()-introStartedAt>=10000)finish()}).catch(error=>onError(error instanceof Error?error.message.replaceAll('_',' '):'MATCH START FAILED'))});schedule(10000,()=>{audio.pause();finish()});return()=>{disposed=true;audio.removeEventListener('loadedmetadata',playIntroSegment);timersRef.current.forEach(t=>window.clearTimeout(t));timersRef.current=[];const current=audioRef.current;if(current){current.pause();current.currentTime=0}audioRef.current=null}},[match.id,session.accessToken])
  return <main className="app mx-vs-intro-root"><section className={`mx-vs-intro stage-${stage.toLowerCase()}`} aria-label="Match introduction"><div className="mx-vs-brand" aria-label="MEGA-X"><img className="mx-vs-brand-logo" src="/ui/landing/logo.avif" alt="MEGA-X"/></div><div className="mx-vs-noise" aria-hidden="true"/><div className="mx-vs-flash-grid" aria-hidden="true"/><div className="mx-vs-speedlines mx-vs-speedlines-left" aria-hidden="true"/><div className="mx-vs-speedlines mx-vs-speedlines-right" aria-hidden="true"/><div className="mx-vs-half mx-vs-half-p1"><div className="mx-vs-energy mx-vs-energy-p1" aria-hidden="true"/><div className="mx-vs-player-tag">X FIGHTER 1</div><h1 className="mx-vs-name mx-vs-name-p1">{match.player1_handle}</h1><div className="mx-vs-role">CHALLENGER</div></div><div className="mx-vs-half mx-vs-half-p2"><div className="mx-vs-energy mx-vs-energy-p2" aria-hidden="true"/><div className="mx-vs-player-tag">X FIGHTER 2</div><h1 className="mx-vs-name mx-vs-name-p2">{match.player2_handle}</h1><div className="mx-vs-role">ACCEPTER</div></div><div className="mx-vs-seam" aria-hidden="true"><span/></div><div className="mx-vs-impact-ring" aria-hidden="true"/><div className="mx-vs-mark" aria-hidden="true"><span>V</span><i/><span>S</span></div><div className="mx-vs-fight" aria-hidden="true">FIGHT!</div><div className="mx-vs-reveal-flash" aria-hidden="true"/></section></main>
}
